...
def get_data(place, forecast_days, kind):
    ...
    #8observaciones en un día
    nr_values = 8*forecast_days # 8 observaciones
    filtered_data = filtered_data[:nr_values]
    if kind == 'Temperature':
        filtered_data = [dict['main']['temp'] for dict in filtered_data] # de igual forma es el contenido de la url
    if kind == 'Sky':
        filtered_data = [dict['weather'][0]['main'] for dict in filtered_data]
    return filtered_data

if __name__ == "__main__":
    print(get_data(place="Quito",forecast_days=3, kind='Temperature'))